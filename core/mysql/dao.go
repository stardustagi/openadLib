...
type Dao interface {
	...
	Where(where string, args ...interface{}) *xorm.Session
	FindAndCount(rowsSlicePtr interface{},
		pageable Pageable, condiBean ...interface{}) (int64, error)

	FindAndCountW(rowsSlicePtr interface{},
		pageable Pageable, wString string, args ...interface{}) (int64, error)
...
func (m *OrmBaseDao) Where(where string, args ...interface{}) *xorm.Session {
	if m.session != nil {
		return m.session.Where(where, args)
	}
	return m.conn.Where(where, args)
}

func (m *OrmBaseDao) Or(where string, args ...interface{}) *xorm.Session {
	if m.session != nil {
		return m.session.Or(where, args)
	}
	// return m.conn.Or(where, args)	未实现
	return nil
}

func (m *OrmBaseDao) In(where string, args ...interface{}) *xorm.Session {
	if m.session != nil {
		return m.session.And(where, args)
	}
	return m.conn.In(where, args)
}

func (m *OrmBaseDao) FindAndCount(rowsSlicePtr interface{},
	pageable Pageable, condiBean ...interface{}) (int64, error) {
	if pageable.Sort() == "" {
		pageable.SetSort("id desc")
	}
	if m.session != nil {
		return m.session.
...
func (m *OrmBaseDao) FindAndCountW(rowsSlicePtr interface{},
	pageable Pageable, wString string, args ...interface{}) (int64, error) {
	if pageable.Sort() == "" {
		pageable.SetSort("id desc")
	}
	if m.session != nil {
		return m.session.Where(wString, args...).
			Limit(pageable.Limit(), pageable.Skip()).
			OrderBy(pageable.Sort()).
			FindAndCount(rowsSlicePtr)
	}
	return m.conn.Where(wString, args...).
		Limit(pageable.Limit(), pageable.Skip()).
		OrderBy(pageable.Sort()).
		FindAndCount(rowsSlicePtr)
}
...
func (m *OrmBaseDao) Limit(limit int, start int) *xorm.Session {
	if m.session != nil {
		return m.session.Limit(limit, start)
	}
	return m.conn.Limit(limit, start)
}
